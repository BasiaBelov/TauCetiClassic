...
ADD_TO_GLOBAL_LIST(/mob/living/carbon/dusty, chief_animal_list)
/mob/living/carbon/dusty
	name = "Dusty"
	real_name = "Dusty"
	icon = 'icons/mob/pets/cat.dmi'
	icon_state = "cat"
	gender = MALE
	desc = "Милый питомец главного врача."

	var/response_help  = "pets"
	var/response_disarm = "bops"
	var/response_harm   = "kicks"

	var/list/speak = list("Мяу!","Мурр","ХСССС!","Мря")
	speak_emote = list("barks", "woofs")
	var/list/emote_hear = list("мяукает")
	var/list/emote_see = list("виляет хвостом", "облизывается", "двигает ушками")
	var/speak_chance = 1
	...
	var/dusty_action = IAN_STANDARD   // Overrides click logic, holds special abilities.
...
/mob/living/carbon/dusty/atom_init()
	reagents = new(1000)
...
/mob/living/carbon/dusty/UnarmedAttack(atom/A)
	..()
	if(dusty_action)
		if(is_busy(A))
			return
...
/mob/living/carbon/dusty/proc/ate_mouse()
...
/mob/living/carbon/dusty/proc/hiccup()
	soap_eaten = max(0, soap_eaten - 1)
	if(prob(33))
		new /obj/effect/bubble_dusty(loc, src)

/obj/effect/bubble_dusty
	name = "Bubble"
	icon = 'icons/effects/effects.dmi'
	icon_state = "bubble"
	anchored = TRUE

/obj/effect/bubble_dusty/atom_init(loc, mob/M)
	. = ..()
...
/obj/effect/bubble_dusty/proc/pop()
...
/mob/living/carbon/dusty/proc/sniff_around()
	if(!client)
		return
...
/mob/living/carbon/dusty/proc/unvisualize_smell(image/I)
	if(!client)
		return
...
//Standard procs, etc.
/mob/living/carbon/dusty/IsAdvancedToolUser()
	return FALSE

/mob/living/carbon/dusty/movement_delay()
	var/tally = speed
...
/mob/living/carbon/dusty/SelfMove(turf/n, direct)
	if(restrained())
...
/mob/living/carbon/dusty/attackby(obj/item/O, mob/user)
	var/chance = 0
...
/mob/living/carbon/dusty/helpReaction(mob/living/carbon/attacker, show_message = TRUE)
	if(health >= config.health_threshold_crit)
		help_shake_act(attacker)
		return
	INVOKE_ASYNC(src, .proc/perform_av, attacker)

/mob/living/carbon/dusty/emp_act(severity)
	if(neck)
		neck.emplode(severity)
	..()

/mob/living/carbon/dusty/ex_act(severity)
	if(!blinded)
		flash_eyes()
...
/mob/living/carbon/dusty/blob_act()
	if (stat != DEAD)
		adjustFireLoss(20)
		updatehealth()
	else
...
/mob/living/carbon/dusty/get_unarmed_attack()
	var/retDam = 3
...
/mob/living/carbon/dusty/is_usable_head(targetzone = null)
	return TRUE

/mob/living/carbon/dusty/is_usable_arm(targetzone = null)
	return FALSE

/mob/living/carbon/dusty/is_usable_leg(targetzone = null)
	return TRUE

/mob/living/carbon/dusty/bullet_act(obj/item/projectile/Proj, def_zone)
	var/chance = 0
	if(head && istype(head,/obj/item/clothing/head/helmet))
		chance += 50
	if(back && istype(back,/obj/item/clothing/suit/armor))
...
/mob/living/carbon/dusty/hitby(atom/movable/AM, datum/thrownthing/throwingdatum)
	if(is_armored(AM, msg = "armored"))
		return
	..()

/mob/living/carbon/dusty/proc/is_armored(atom/movable/AM, luck = 50, msg = "dodges")
	var/chance = 0
	if(head && istype(head,/obj/item/clothing/head/helmet))
		chance += luck
	if(back && istype(back,/obj/item/clothing/suit/armor))
...
/mob/living/carbon/dusty/toggle_throw_mode()
	return // nope.avi

/mob/living/carbon/dusty/throw_mode_on()
	return

/mob/living/carbon/dusty/throw_mode_off()
	return

/mob/living/carbon/dusty/gib()
	if(butcher_results)
...
/mob/living/carbon/dusty/say(message)
	if(stat != CONSCIOUS)
		return
...
/mob/living/carbon/dusty/get_scrambled_message(message, datum/language/speaking = null)
	if(!speak.len)
		return null
	return pick(speak)

/mob/living/carbon/dusty/verb/dusty_sit()
	set name = "Ian Sit"
	set category = "IC"
	dusty_sit = !dusty_sit
	to_chat(src, "<span class='notice'>You are now [dusty_sit ? "sitting" : "getting up"].</span>")
	update_canmove()
