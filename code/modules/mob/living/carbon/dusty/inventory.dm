/mob/living/carbon/dusty/show_inv(mob/user)
...
/mob/living/carbon/dusty/get_active_hand()
	return mouth

//Returns the thing in our inactive hand (errr... mouth!)
/mob/living/carbon/dusty/get_inactive_hand()
	return mouth

//Drops the item in our active hand (errr... mouth!)
/mob/living/carbon/dusty/drop_item(atom/Target)
	return drop_from_inventory(mouth, Target)

/mob/living/carbon/dusty/restrained()
	if(handcuffed || facehugger) // Oh wow, whats this on my faaaaaaace?
		return TRUE
	return FALSE

/mob/living/carbon/dusty/equip_to_slot(obj/item/W, slot)
	if(!slot)
		return
	if(!istype(W))
		return
...
/mob/living/carbon/dusty/put_in_active_hand(obj/item/W)
...
/mob/living/carbon/dusty/put_in_inactive_hand(obj/item/W)
	return put_in_active_hand(W)

// dusty have only one hand
/mob/living/carbon/dusty/put_in_r_hand(obj/item/W)
	return put_in_active_hand(W)

/mob/living/carbon/dusty/put_in_l_hand(obj/item/W)
	return put_in_active_hand(W)

/mob/living/carbon/dusty/put_in_hands(obj/item/W)
	if(!W)
		return FALSE
...
/mob/living/carbon/dusty/u_equip(obj/item/W)
	if (W == head)
		facehugger = FALSE
		head = null
	else if (W == neck)
...
/mob/living/carbon/dusty/proc/update_cat_ability()
	name = real_name
	desc = initial(desc)
	speak = list("Мяу!","Мурр","ХСССС!","Мря")
	speak_emote = list("barks", "woofs")
	emote_hear = list("мяукает")
	emote_see = list("виляет хвостом", "облизывается", "двигает ушками")
