...
/datum/role/traitor/Greet(greeting = GREET_DEFAULT, custom)
	...
	switch(greeting)
		if (GREET_ROUNDSTART)
			to_chat(antag.current, "[bicon(logo, css = "style='position:relative; top:10;'")] <span class='danger'>Ты агент Синдиката. Предатель.</span>")
		if (GREET_AUTOTRAITOR)
			to_chat(antag.current, "[bicon(logo, css = "style='position:relative; top:10;'")] <span class='danger'>Теперь вы Предатель.<br>Ваша память проясняется, когда вы вспоминаете, что являетесь спящим агентом Синдиката. Пришло время отдать им свой долг. </span>")
		if (GREET_LATEJOIN)
			to_chat(antag.current, "[bicon(logo, css = "style='position:relative; top:10;'")] <span class='danger'>Вы Предатель.<br>Как агент Синдиката, ваша цель - внедриться в команду и достичь своих целей любой ценой.</span>")
		if (GREET_SYNDBEACON)
			to_chat(antag.current, "[bicon(logo, css = "style='position:relative; top:10;'")] <span class='danger'>Вы вступили в ряды Синдиката и таким образом стали Предателем НаноТрейзен!</span>")
		else
			to_chat(antag.current, "[bicon(logo, css = "style='position:relative; top:10;'")] <span class='danger'>Вы Предатель.</span>")
...
/datum/role/traitor/syndcall/Greet(greeting, custom)
	..()
	to_chat(antag.current, "<span class='userdanger'> <B>ВНИМАНИЕ:</B> Вы слышите сигнал от Синдиката...</span>")
